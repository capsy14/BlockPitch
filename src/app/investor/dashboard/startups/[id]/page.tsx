...
import {
  StartupActions,
  ConnectWithFounder
} from "@/components/StartupActions";
import { MdOutlineMail } from "react-icons/md";
import { Card, CardContent, CardHeader, CardTitle } from "@/components/ui/card";
import { Separator } from "@/components/ui/separator";
import { MessageSquare,MessageCircleMore,Link2  , DollarSign, Video } from "lucide-react";
...
export default async function StartupDetail({
  ...
  const startup = await TypedStartup.findById(id).lean().exec();
...
<Link href={`/investor/dashboard/messages/chat?recipientId=${startup.founderUserId }`}>                    <Button
                      className="w-full mt-2 flex items-center justify-center gap-2"
                      variant="secondary"
                    >
                        ...
                        <div className="mt-2 space-y-1 text-sm text-muted-foreground">
                          {startup.cofounderEmail && (
                            <p>
                              Email:{" "}
                              <Link
                                href={`mailto:${startup.cofounderEmail}`}
                                className="text-primary hover:underline"
                              >
                                {startup.cofounderEmail}
                              </Link>
