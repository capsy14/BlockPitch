...
export function InvestmentHistory() {
  ...
  date: new Date(tx.timestamp || tx.date || Date.now()).toLocaleDateString("en-US", {
    month: "short",
    year: "2-digit",
  }),
    ...
    >
      <div className="flex items-center gap-4 w-full md:w-auto">
        <Avatar className="h-10 w-10 flex-shrink-0">
          ...
          <AvatarFallback>{tx.startupName?.charAt(0) || "S"}</AvatarFallback>
        </Avatar>
        <div className="flex-1 min-w-0">
          <p className=" truncate font-bold text-black">{tx.startupName || "Startup"}</p>
          <p className="text-sm text-muted-foreground truncate">{tx.startupId}</p>
        </div>
      </div>
      ...
      <div className="flex-1 min-w-0">
        <p className="font-bold text-black text-sm">Recipient</p>
